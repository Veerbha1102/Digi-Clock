...
function populateTimezoneSelectors() {
    const selectList = document.getElementById('timezone-select-list');
    const listContainer = document.getElementById('timezone-list-container');
    
    // Popular timezones for quick select
    const popularZones = [
        'UTC', 'America/New_York', 'America/Chicago', 'America/Denver', 'America/Los_Angeles',
        'Europe/London', 'Europe/Paris', 'Europe/Berlin', 'Asia/Tokyo', 'Asia/Shanghai',
        'Asia/Hong_Kong', 'Asia/Dubai', 'Asia/Singapore', 'Asia/Kolkata', 'Australia/Sydney'
    ];
    ...
    // Populate full timezone list
    renderTimezoneList(ALL_TIMEZONES);
    
    // Setup timezone search and filter
    document.getElementById('timezone-search').addEventListener('input', (e) => {
        const filtered = ALL_TIMEZONES.filter(tz => 
            tz.toLowerCase().includes(e.target.value.toLowerCase())
        );
        renderTimezoneList(filtered);
    });
    
    document.getElementById('timezone-filter').addEventListener('input', (e) => {
        const filtered = ALL_TIMEZONES.filter(tz => 
            tz.toLowerCase().includes(e.target.value.toLowerCase())
        );
        renderTimezoneList(filtered);
    ...
    // Auto-populate timezone input on select list change
    document.getElementById('timezone-select-list').addEventListener('change', (e) => {
        if (e.target.value) {
            document.getElementById('timezone-input').value = e.target.value;
        }
    });
    
    // Update color label
    document.getElementById('timezone-color').addEventListener('input', (e) => {
...
function renderTimezoneList(timezones) {
    ...
    timezones.slice(0, 200).forEach(tz => {
        const item = document.createElement('div');
        item.className = 'timezone-item';
        item.textContent = tz;
        item.onclick = () => {
            document.getElementById('timezone-input').value = tz;
            document.getElementById('timezone-select-list').value = tz;
        };
...
function addCustomTimezone() {
    ...
    let timezone = document.getElementById('timezone-input').value.trim();
    ...
    // If timezone doesn't contain '/', it might be just the region
    if (!timezone.includes('/')) {
        const found = ALL_TIMEZONES.find(tz => tz.toLowerCase().includes(timezone.toLowerCase()));
        if (found) timezone = found;
        else {
